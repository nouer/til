...
## 書評
Gitを使い始めたら、必ず目を通しておきたい一冊。ローカルでソロで開発する上でGitを利用することから、リモートで複数人で操作する状況や、起きうるコンフリクトなどをわかりやすく説明している。コマンドラインとGUIの両方の視点で記述されている。ありがちだが、演習に回答がないので、その点は不満に思った。また、書籍の内容は古めなので、GUIなどに関しては時代遅れ感が否めない。それでもGitを使い始めたら最初に目を通しておきたい書籍。
